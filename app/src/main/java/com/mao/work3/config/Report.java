...
public class Report
{
	...
	public void total(Month month)
	{
		for (int i=1; i <= 31; i++)
		{
			Day day = month.getDay(i);
			
			if (null != day && !day.getShift().equals(Shift.REST))
			{
				...
				{
					//获取倍数
					String rate_str = note.getRate().getRateName();
					int rate_length = rate_str.length();
					float rate = Float.parseFloat(rate_str.substring(0, rate_length - 1));
					//获取小时
					String shour = note.getHour().getHourName();
					int length = shour.length();
					float hour = Float.parseFloat(shour.substring(0, length - 1));
					
					if(note.getFake().equals(Fake.NORMAL)){

						//平时加班
						if (Rate.ONE_AND_HALF.equals(note.getRate()))
						{
							String key = "平时加班(时)";
							this.set(key, this.get(key) + hour);
						}
						//周末加班
						else if (Rate.TWO.equals(note.getRate()))
						{
							String key = "周末加班(时)";
							this.set(key, this.get(key) + hour);
						}
						//节假日加班
						else if (Rate.THREE.equals(note.getRate()))
						{
							String key = "节假日加班(时)";
							this.set(key, this.get(key) + hour);
						}
						...
						if (Fake.TAKEOFF.equals(note.getFake()))
						{
							String key = "调休(时)";
							this.set(key, this.get(key) + hour);
							//rateXhour += rate * hour;
				...
				if(day.getShift().equals(Shift.MIDDLE))
				{
					String key = "中班天数(天)";
					if(f<4)this.set(key, this.get(key) + 1);

				}
				
				if(day.getShift().equals(Shift.NIGHT))
				{
					String key = "夜班天数(天)";
					if(f<4)this.set(key, this.get(key) + 1);
				}
	...
	/*	for (int i=1; i <= 31; i++)
		{
			...
			{
				//获取倍数
				String rate_str = month.getDay(i).getNotes().get(0).getRate().getRateName();
				int rate_length = rate_str.length();
				float rate = Float.parseFloat(rate_str.substring(0, rate_length - 1));
				//获取小时
				String shour = month.getDay(i).getNotes().get(0).getHour().getHourName();
				int length = shour.length();
				float hour = Float.parseFloat(shour.substring(0, length - 1));
				//不是休班
				...
				{
					//加班
					if (Fake.NORMAL.equals(month.getDay(i).getNotes().get(0).getFake()))
					{
						//平时加班
						if (Rate.ONE_AND_HALF.equals(month.getDay(i).getNotes().get(0).getRate()))
						{
							String key = "平时加班(时)";
							this.set(key, this.get(key) + hour);
						}
						//周末加班
						else if (Rate.TWO.equals(month.getDay(i).getNotes().get(0).getRate()))
						{
							String key = "周末加班(时)";
							this.set(key, this.get(key) + hour);
						}
						//节假日加班
						else if (Rate.THREE.equals(month.getDay(i).getNotes().get(0).getRate()))
						{
							String key = "节假日加班(时)";
							this.set(key, this.get(key) + hour);
						}
					...
					//调休
					if (Fake.TAKEOFF.equals(month.getDay(i).getNotes().get(0).getFake()))
					{
						String key = "调休(时)";
						this.set(key, this.get(key) + hour);
//						rateXhour += rate * hour;
					...
					//事假
					if (Fake.LEAVE.equals(month.getDay(i).getNotes().get(0).getFake()))
					{
						String key = "事假(时)";
						this.set(key, this.get(key) + hour);
					}
					//病假
					if (Fake.SICK.equals(month.getDay(i).getNotes().get(0).getFake()))
					{
						String key = "病假(时)";
						this.set(key, this.get(key) + hour);
					}
					//年假
					if (Fake.PAID.equals(month.getDay(i).getNotes().get(0).getFake()))
					{
						String key = "年假(时)";
						this.set(key, this.get(key) + hour);
					}
