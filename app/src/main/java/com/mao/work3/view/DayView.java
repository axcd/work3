		...
		if (isDraw())
		{
			...
			if (null != day)
			{
				...
				if (null != day.getShift())
				{
					...
					//设置绘制请假字符
					if (null != note.getFake() && !note.getFake().equals(Fake.NORMAL))
					{
						shift = note.getFake().getFakeName();
					}
				...
				if (null != note.getHour())
				{
					String hour = note.getHour().getHourName();
					int hour_color = Color.parseColor("#80000FFF");
					int hour_bkgcolor = Color.parseColor("#20FF0000");

					//分别绘制加班倍数下面背景颜色
					if (note.getRate().equals(Rate.ONE_AND_HALF))
					{
						hour_color = Color.parseColor("#900000FF");
					}
					else if (note.getRate().equals(Rate.TWO))
					{
						hour_color = Color.parseColor("#90FF0000");
					}
					else if (note.getRate().equals(Rate.THREE))
					{
					...
					//如果为零不显示小时
					if(note.getHour().equals(Hour.ZERO))
					{
						hour = "";
					}
